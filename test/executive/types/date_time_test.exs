defmodule Executive.Types.DateTimeTest do
  ...
  describe "parse/3" do
    test "parses iso 8601" do
      ...
      assert DateTimeType.parse([], nil, raw) == {:ok, expected}
    end
    ...
    test "error if format invalid" do
      ...
      assert DateTimeType.parse([], nil, raw) == :error
    end

    test "error if date invalid" do
      raw = "2023-12-32T00:00:00Z"
      assert DateTimeType.parse([], nil, raw) == {:error, "invalid date"}
    end

    test "error if time invalid" do
      raw = "2023-01-01T24:00:00Z"
      assert DateTimeType.parse([], nil, raw) == {:error, "invalid time"}
    end

    test "error if missing offset" do
      raw = "2023-01-01T00:00:00"
      assert DateTimeType.parse([], nil, raw) == {:error, "missing offset"}
    end
  end
